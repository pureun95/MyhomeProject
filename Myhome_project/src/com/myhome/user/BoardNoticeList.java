...
/**
 * 공지사항 게시판의 리스트를 조회하는 클래스입니다.
 * @author 노푸른
 *
 */
@WebServlet("/Myhome/user/boardnoticelist.do")
public class BoardNoticeList extends HttpServlet {
	
	/**
	 * get 방식으로 공지사항 게시글 목록 데이터를 호출하는 메소드입니다.
	 */
	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<BoardNoticeDTO> list = dao.list(map);
		...
		if ( n == 1 ) {
			...
			pagebar += String.format("<li>"
					+ "            <a href=\"/Myhome_project/Myhome/user/boardnoticelist.do?page=%d\" aria-label=\"Previous\">"
					+ "                <span aria-hidden=\"true\">&laquo;</span>"
					+ "            </a>"
					+ "        </li>", n - 1);
		...
		while (!(loop > blockSize || n > totalPage)) {
			...
			pagebar += String.format("<a href=\"/Myhome_project/Myhome/user/boardnoticelist.do?page=%d\">%d</a></li>", n, n);
		...
		if (n > totalPage) { //마지막에 끝난 n이 토탈페이지보다 크냐? 나의 경우엔 지금 n이 29냐? > 링크 안걸린 애를 만들기.
			...
			pagebar += String.format("<li>"
					+ "            <a href=\"/Myhome_project/Myhome/user/boardnoticelist.do?page=%d\" aria-label=\"Next\">"
					+ "                <span aria-hidden=\"true\">&raquo;</span>"
					+ "            </a>"
					+ "        </li>", n);
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/views/user/board-notice-list.jsp");
