...
//		System.out.println(totalPage);
		...
		if ( n == 1 ) {
			...
			pagebar += String.format("<li>"
					+ "            <a href=\"/Myhome_project/Myhome/user/boardcommunitylist.do?page=%d\" aria-label=\"Previous\">"
					+ "                <span aria-hidden=\"true\">&laquo;</span>"
					+ "            </a>"
					+ "        </li>", n - 1);
		...
		while (!(loop > blockSize || n > totalPage)) {
					...
					pagebar += String.format("<a href=\"/Myhome_project/Myhome/user/boardcommunitylist.do?page=%d\">%d</a></li>", n, n);
		...
		//다음 10페이지로 이동
				if (n > totalPage) { //마지막에 끝난 n이 토탈페이지보다 크냐? 나의 경우엔 지금 n이 29냐? > 링크 안걸린 애를 만들기.
					...
					pagebar += String.format("<li>"
							+ "            <a href=\"/Myhome_project/Myhome/user/boardcommunitylist.do?page=%d\" aria-label=\"Next\">"
							+ "                <span aria-hidden=\"true\">&raquo;</span>"
							+ "            </a>"
							+ "        </li>", n);
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/views/user/board-community-list.jsp");
