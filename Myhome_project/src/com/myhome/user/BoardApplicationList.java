...
public class BoardApplicationList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<BoardApplicationDTO> list = dao.list(map);
		
		//1.5
		for (BoardApplicationDTO dto : list) {
			//날짜에서 년월일만 잘라내기
		...
		if ( n == 1 ) {
			...
			pagebar += String.format("<li>"
					+ "            <a href=\"/Myhome_project/Myhome/user/boardapplicationlist.do?page=%d\" aria-label=\"Previous\">"
					+ "                <span aria-hidden=\"true\">&laquo;</span>"
					+ "            </a>"
					+ "        </li>", n - 1);
		...
		while (!(loop > blockSize || n > totalPage)) {
			...
			pagebar += String.format("<a href=\"/Myhome_project/Myhome/user/boardapplicationlist.do?page=%d\">%d</a></li>", n, n);
		...
		if (n > totalPage) { //마지막에 끝난 n이 토탈페이지보다 크냐? 나의 경우엔 지금 n이 29냐? > 링크 안걸린 애를 만들기.
			...
			pagebar += String.format("<li>"
					+ "            <a href=\"/Myhome_project/Myhome/user/boardapplicationlist.do?page=%d\" aria-label=\"Next\">"
					+ "                <span aria-hidden=\"true\">&raquo;</span>"
					+ "            </a>"
					+ "        </li>", n);
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/views/user/board-application-list.jsp");
